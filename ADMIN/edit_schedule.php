<?php 
    ...
    $id =  $_GET['id'];
    $sql="select * from schedule where idSchedule = $id";
    ...
    while($row = $result->fetch_assoc())
    {
        ...
        $sub_workout_type = $row['sub_workout_type'];
        $timing = $row['timing'];
        $day = $row['day'];
    }
?>
...
<html lang="en">
  ...
  <head>
    <meta charset="UTF-8">
    <title>Edit Schedule|Fitness Hub Admin Template
    </title>
  ...
  <body>
    <input type="hidden" value="<?php echo $sub_workout_type; ?>" id="sub_workout_type" name="">
    ...
    <aside class="right-side right-padding" > 
      ...
      <section class="content-header">
        ...
        <ol class="breadcrumb">
          ...
          <li>
            <a href="add_trainer_schedule.php" class="activated">Edit Schedule
            </a>
          </li>
        </ol>
      </section>
                        ...
                        <div class="form-group" id="exercise_cont" style="display: none;">
                          ...
                          <?php
                            ...
                            <select id="sel_excer" class="form-control js-states select2" name="idExercise">
                              ...
                              {
                                if( $row['idExercise'] == $sub_workout_type)
                                {
                                  $selected = "selected";
                                }
                              ?>
                              <option value="<?php echo $row['idExercise'] ;?>" <?php echo $selected; ?> >
                                <?php echo $row['name']; ?>
                              </option>
                              <?php
                              }
                    ...
                    <div class="form-actions">
                      <div class="row">
                        <div class="col-md-offset-3 col-md-9">
                          <input type="button" class="btn btn-primary" value="Edit" name="edit" onclick="EditSchdule()"> &nbsp;
...
<script type="text/javascript">
  ...
  {
    $('#select_wo').val('<?php echo $workout_type; ?>');
    $('#select23').val('<?php echo $timing; ?>');
    $('#day_sel').val('<?php echo $day; ?>');

    //alert($('#select_user_type').val());

    
    if($('#sub_workout_type').val() == 1)
    {
      $('#exercise_cont').show();
      $('#activity_cont').hide();
      $('#sel_excer').val($('#sub_workout_type').val());
    }
    else
    {
      $('#exercise_cont').hide();
      $('#activity_cont').show();
      $('#sel_act').val($('#sub_workout_type').val());
    }
  }

  function EditSchdule()
  {
    $.post('../services/edit_schedule.php',{
