...
<div class="super_container">
   ...
   $ex1 = $ex->fetch_assoc();
   ?>		
	<a class="btn btn-warning ml-5" href="../ADMIN/uploads/diet/<?php echo $ex1['Diet_chart']; ?>"><img src="../ADMIN/img/pdf_icon.png" style="width: 35px; margin-right: 10px;" />Checkout Your Diet Plans</a>

	<!-- Timetable -->

	<div class="timetable">
		<div class="parallax_background parallax-window" data-parallax="scroll" data-image-src="images/d11.jpeg" data-speed="0.8"></div>
	...
	<div class="extra d-flex flex-column align-items-center justify-content-center text-center">
		<div class="background_image" style="background-image:url(images/extra_wide.jpg);height:100%"></div>
			<div class="container">
				<div class="row">
					<div class="col-lg-6">
						<div class="section_title_container">
							<div class="section_title">FeedBack</div>
						</div>
						<br/><br/><br/>
						<form method="POST" action="../services/feedback.php" >
							<div class="row">
								<div class="col-md-6">
									<textarea type="text" name="discription" class="form-control" style="height: 100px;" placeholder="Enter Your Feedback.."></textarea>		
								</div>
								<div class="col-md-6">
									<input type="submit" value="Submit" class="btn btn-warning btn-sm">
								</div>
								<div class="col-md-6" style="margin-left:900px">
									<a href="attendance.php" class="btn btn-warning btn-sm">View Attendance</a>
								</div>
								<br/>
								<div class="col-md-6" style="margin-left:900px">
            ...
            success: function (response) {
	            ...
	            console.log(obj);
	               ...
	               for(var i=0;i<obj.length;i++)
	               {
	               		if(obj[i].Day == "Monday")
	            		{
		               		$('#mon_cont').append('<div class="tt_class grid-item weight_loss"><div class="tt_class_title">'+obj[i].name+'</div><div class="tt_class_instructor">'+obj[i].First_name+" "+obj[i].Last_name +'</div></div>');
		               	}
		               	else if(obj[i].Day == "Tuesday")
	            		{
	            			$('#tue_cont').append('<div class="tt_class grid-item weight_loss"><div class="tt_class_title">'+obj[i].name+'</div><div class="tt_class_instructor">'+obj[i].First_name+" "+obj[i].Last_name +'</div></div>');
	            		}
	            		else if(obj[i].Day == "Wednesday")
	            		{
	            			$('#wed_cont').append('<div class="tt_class grid-item weight_loss"><div class="tt_class_title">'+obj[i].name+'</div><div class="tt_class_instructor">'+obj[i].First_name+" "+obj[i].Last_name +'</div></div>');
	            		}
	            		else if(obj[i].Day == "Thursday")
	            		{
	            			$('#thu_cont').append('<div class="tt_class grid-item weight_loss"><div class="tt_class_title">'+obj[i].name+'</div><div class="tt_class_instructor">'+obj[i].First_name+" "+obj[i].Last_name +'</div></div>');
	            		}
	            		else if(obj[i].Day == "Friday")
	            		{
	            			$('#fri_cont').append('<div class="tt_class grid-item weight_loss"><div class="tt_class_title">'+obj[i].name+'</div><div class="tt_class_instructor">'+obj[i].First_name+" "+obj[i].Last_name +'</div></div>');
	            		}
	            		else if(obj[i].Day == "Saturday")
	            		{
	            			$('#sat_cont').append('<div class="tt_class grid-item weight_loss"><div class="tt_class_title">'+obj[i].name+'</div><div class="tt_class_instructor">'+obj[i].First_name+" "+obj[i].Last_name +'</div></div>');
	            		}
	            		else if(obj[i].Day == "Sunday")
	            		{
	            			$('#sun_cont').append('<div class="tt_class grid-item weight_loss"><div class="tt_class_title">'+obj[i].name+'</div><div class="tt_class_instructor">'+obj[i].First_name+" "+obj[i].Last_name +'</div></div>');
	            		}
	               }
